...
-(void) drawLevel{
    ...
    ropeAtCenter=[[Rope alloc] init];
    [self.scene addChild:ropeAtCenter];
    
    ropeAtCenter.ropeName=namePunchRope;
    ropeAtCenter.ropeLength=5;//ropeLength;
    ropeAtCenter.ropePosition=centerPosition;
    [ropeAtCenter drawRopeWithWreckingBall];
    
    CGPoint ropePosition = CGPointMake(centerPosition.x - [ropeAtCenter getRopeWidth]*3, centerPosition.y);
    ...
    ropePosition = CGPointMake(centerPosition.x + [ropeAtCenter getRopeWidth]*3, centerPosition.y);
    for(u_int8_t r=1;r<=numberOfRopes/2;r++){
        Rope *rope= [[Rope alloc] init];
        [self.scene addChild:rope];
        //[ropeArray addObject:rope];
...
/*-(void) didBeginContact:(SKPhysicsContact *)contact{
    ...
    if ((contact.bodyA.categoryBitMask==WreckingBallCategory && contact.bodyB.categoryBitMask==BallCategory) ||
        ...
        NSLog(@"Contact: name is %@",wreckingBallBody.node.name);
        
        if(wreckingBallBody.node.name==namePunchRope){
            NSLog(@"Contact: name is %@",wreckingBallBody.node.name);
...
-(void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    [ropeAtCenter touchesBegan:touches withEvent:event];
